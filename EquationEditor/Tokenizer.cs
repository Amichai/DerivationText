...
namespace EquationEditor {
    class Tokenizer {
        ...
        public List<IToken> Tokenize(string text) {
            ...
            var tokens = addImpliedMultiplication(tokenStrings);

            return ConvertToPostfixed(tokens);
        }

        private static List<IToken> addImpliedMultiplication(List<string> tokenStrings) {
            var tokens = tokenStrings.Select(i => TokenLibrary.Create(i)).ToList();
            List<IToken> keywords = new List<IToken>();
            for (int i = 0; i < tokens.Count - 1; i++) {
                var a = tokens[i];
                var b = tokens[i + 1];
                if ((a.Type == TokenType.keyword && b.Type == TokenType.keyword) ||
                    (a.Value == ")" && b.Type == TokenType.keyword)) {
                    var op = new ImpliedMultiplicationInfix();
                    tokens.Insert(i + 1, op);
                }
            }
            return tokens;
        }
